...
m <- mongo("messages_R", url = "mongodb://cparra:i.}4rF-e(-mUE@127.0.0.1/bdd_grp4?authSource=admin")


# Combien de publications chaque utilisateur a-t-il faites
pub_par_ut_R <- m$aggregate('[
...
tot_publications_R <- m$aggregate('[
            {"$group": {"_id":"$username","publications": {"$sum": 1 } } }, 
            {"$sort": {"publications": -1 } },
...
list_user_R <- m$distinct("username")
